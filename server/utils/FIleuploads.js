...
export const Fileupload = (size , img , res , req , model , img_field) => {
    ...
    const upload_path = path.join(path.resolve(), "public/uploads", newFilename + filext);
        ...
        try {
            ...
            if (dp_prev && dp_prev[img_field]) {
                ...
                if (!file_prev.includes("defaults")) {
                    ...
                    const file_path = path.join(path.resolve(), `public/uploads/${file_name}`);
            ...
            const updatedData = await model.findOneAndUpdate(
                { belongsto: id },
                { [img_field]: `${process.env.SERVER_URI}/uploads/${newFilename}${filext}` },
                { new: true } 
            );
